...
import { WORKSHOP_ACHIEVEMENTS_FR } from "../data_fr";
...
export function DashboardHeader({ achievements, assemblyProgressPercent, lang }: Props) {
    ...
    <div className="bg-[#111827] border border-[#374151] rounded-2xl p-6 md:p-8 shadow-xl max-w-7xl mx-auto mb-8 animate-fade-in">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div className="flex-1">
          ...
          <h1 className="text-3xl md:text-5xl font-sans font-bold text-white tracking-tight leading-tight">
            {t.header_title_main} <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-orange-500">{t.header_title_sub}</span>
          </h1>
          <p className="mt-3 text-gray-400 text-sm md:text-base max-w-2xl leading-relaxed">
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto shrink-0 select-none">
          ...
          <div className="bg-gray-900/60 border border-gray-800 rounded-xl p-4 flex items-center gap-4 flex-1 sm:flex-initial min-w-[250px]">
            ...
            <div>
              <div className="text-[10px] text-gray-400 uppercase tracking-wider font-mono">{t.header_badges_unlocked}</div>
              <div className="text-base font-bold text-white mb-0.5 flex items-center gap-1.5">
                <Award className="w-4 h-4 text-amber-400" /> {unlockedCount} / {achievements.length} {lang === "fr" ? "Débloqués" : "Unlocked"}
              </div>
              <div className="text-[10px] text-gray-500">{t.header_badges_tip}</div>
            </div>
          </div>
          ...
          <div className="bg-gray-900/60 border border-gray-800 rounded-xl p-4 flex items-center gap-4 flex-1 sm:flex-initial min-w-[250px]">
            ...
            <div>
              <div className="text-[10px] text-gray-400 uppercase tracking-wider font-mono">{t.header_checklist_unlocked}</div>
              <div className="text-base font-bold text-white mb-0.5 flex items-center gap-1.5">
                <ClipboardCheck className="w-4 h-4 text-emerald-400" /> {assemblyProgressPercent}% {lang === "fr" ? "Complété" : "Complete"}
              </div>
              <div className="text-[10px] text-gray-500">{t.header_track_build}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mt-8">
        {achievements.map((a) => {
          ...
          // If French, retrieve French achievement descriptions
          const FrenchTemplate = WORKSHOP_ACHIEVEMENTS_FR.find((fa) => fa.id === a.id);
          const displayTitle = lang === "fr" && FrenchTemplate ? FrenchTemplate.title : a.title;
          const displayDescription = lang === "fr" && FrenchTemplate ? FrenchTemplate.description : a.description;

          return (
            <div
              key={a.id}
              className={`p-4 rounded-xl border transition-all duration-300 flex flex-col items-start gap-3.5 h-full ${
                isUnlocked
                  ? "bg-gradient-to-br from-gray-900 to-gray-800 border-amber-500/50 shadow-md shadow-amber-500/5 scale-[1.02]"
